...
export default function ExerciseCard(props) {
 ...
 return (
  <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
   ...
   <div className="grid grid-col sm:grid-cols-4 sm:place-items-center gap-2">
    {['reps', 'rest', 'tempo'].map(info => {
     return (
      <div key={info} className="flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full">
       <h3 className="capitalize text-slate-blue-900 text-sm">{info === 'reps' ? `${exercise.unit}` : info}</h3>
       <p className="font-medium">{exercise[info]}</p>
      </div>
     )
    })}
    <button onClick={handleSetIncrement} className="flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-blue-900 w-full duration-200">
